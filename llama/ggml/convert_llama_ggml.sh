...
python "${script_dir}/llama.cpp/convert.py" "${input_dir}" --outfile "${output_dir}/llama-${model_size,,}.ggmlv3.f16.bin"

for q in "${qs[@]}"; do
  (cd "${script_dir}/llama.cpp" && ./quantize "${output_dir}/llama-${model_size,,}.ggmlv3.f16.bin" "${output_dir}/llama-${model_size,,}.ggmlv3.${q}.bin" "${q}")
done
